...
def Iteration(index, matrix_op, U, V):
    ...
    print(f'Рассчитанное значение: {np.round(x_predict, 2)}')
    print(f'Квадратичная ошибка: {np.round(err, 2)}')
    print(f'Обновленные значения скрытых факторов пользователя : {np.round(U_row, 2)}')
    print(f'Обновленные значения скрытых факторов объектов : {np.round(V_row, 2)}')
